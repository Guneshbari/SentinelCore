...
import { alerts, events, formatTimestamp, getActiveAlerts, getAcknowledgedAlerts } from '../data/mockData';
    ...
    <div className="space-y-5">
        ...
        <div className="flex glass-panel rounded-lg overflow-hidden">
          <button
            ...
            className={`px-4 py-2 text-xs font-medium transition-colors ${
              tab === 'active' ? 'bg-accent-blue text-white shadow-[0_0_12px_rgba(10,132,255,0.3)]' : 'text-text-secondary hover:text-text-primary'
            }`}
          >Active ({activeAlerts.length})</button>
          <button
            onClick={() => setTab('acknowledged')}
            className={`px-4 py-2 text-xs font-medium transition-colors ${
              tab === 'acknowledged' ? 'bg-accent-blue text-white shadow-[0_0_12px_rgba(10,132,255,0.3)]' : 'text-text-secondary hover:text-text-primary'
            }`}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        {summaryCards.map(({ severity, cssClass }) => (
          <div key={severity} className={`glass-panel ${cssClass} rounded-lg px-4 py-3`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        {/* Alert Timeline */}
        <div className="lg:col-span-3 glass-panel rounded-xl p-4 space-y-1 animate-fade-in">
            ...
            <button
              ...
              className={`w-full text-left flex items-start gap-3 px-3 py-3 rounded-lg transition-all ${
                selectedAlert?.alert_id === alert.alert_id
                  ? 'bg-accent-blue/10 border border-accent-blue/30 shadow-[0_0_8px_rgba(10,132,255,0.1)]'
        ...
        {/* Alert Details + Correlation */}
        <div className="lg:col-span-2 glass-panel rounded-xl p-5 animate-fade-in">
          ...
          {selectedAlert ? (
            <div className="space-y-4">
              ...
              {correlationData.length > 0 && (
                <div>
                  <h5 className="text-[10px] font-semibold text-text-muted uppercase tracking-wider mb-2">Correlated Events — {selectedAlert.hostname}</h5>
                  <ResponsiveContainer width="100%" height={130}>
                    <ComposedChart data={correlationData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                      ...
                      <Area type="monotone" dataKey="cpu" stroke="#0a84ff" fill="url(#cpuAlert)" strokeWidth={1.5} name="CPU %" />
              ...
              {!selectedAlert.acknowledged && (
                <div className="flex gap-2 pt-2">
                  <button className="flex items-center gap-1.5 px-4 py-2 bg-accent-blue hover:bg-accent-blue-hover text-white text-xs font-medium rounded-lg transition-colors shadow-[0_0_12px_rgba(10,132,255,0.2)]">
